...
//Template literals
//used to include the dynamic values to the static text  or to console 
...
// "JavaScript code to fetch data from an API and display it on an HTML page"


let getlistofproductsElement=document.querySelector('.list-of-products')

function renderProducts(getProducts){
    getlistofproductsElement.innerHTML=getProducts.map(singleProduct=> `<p>${singleProduct.title}</p>`).join(" ")
   getProducts.map(singleProduct=> console.log(singleProduct.title)).join(" ")
}

async function fectListOfProducts(){
    try{
